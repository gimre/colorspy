( function ( scope, d, n ) {
    ...
    scope.utils = {
        ...
        extractColorMap: function ( image, callback ) {
            var cores = n.hardwareConcurrency || 2,
                pixels = utils.getImagePixels32( image );

            utils.runJobs( cores, pixels, callback );
        },
        ...
        runJobs: function ( count, pixels, callback ) {
            ...
            for ( var i = 0; i < count; i ++ ) {
                var worker = new Worker( 'extractor.js' ),
                    begin  = chunkSize * i | 0,
                    end    = chunkSize * ( i + 1 ) | 0;

                utils.startJob( worker, pixels.buffer.slice( begin * 4, end * 4 ) );

                worker.onmessage = function ( event ) {
                    utils.mergeMaps( colorMap, event.data );
                    if ( ! -- count ) {
                        callback( colorMap );
                    }
                }
